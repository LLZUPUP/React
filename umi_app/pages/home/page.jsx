...
export default class extends Component {
  ...
  loadRestaurantData({ latitude, longitude },rank_id='') {
    fetch(`/restapi/shopping/v3/restaurants?latitude=${latitude}&longitude=${longitude}&offset=${this.rests.length}limit=8&extras[]=activities&extras[]=tags&extra_filters=home&rank_id=&terminal=h5&rank_id=${rank_id}`).then(res=>{
      if(res.status===200) {
        res.json().then(data=>{
    ...
    return (
      <div className={styles.home}>
        ...
        <StickyContainer>
          ...
          <ListView dataSource={this.dataSource} renderFooter={()=> (<div style={{padding: 30,textAlign: 'center'}}>{this.state.isLoading?'加载中...':'暂无更多'}</div>)} renderRow={this.renderRow} pageSize={4} useBodyScroll scrollRenderAheadDistance={500} onEndReached={this.onEndReached} onEndReachedThreshold={10}/>
